...
const legacyIeltsV3=legacy=>{legacy.domains.ielts.databaseVersion=3;delete legacy.domains.ielts.stores[IELTS_STORE_NAMES.testBlueprints];delete legacy.domains.ielts.stores[IELTS_STORE_NAMES.testRuns];legacy.manifest.stores=legacy.manifest.stores.filter(row=>!(row.owner==='ielts'&&[IELTS_STORE_NAMES.testBlueprints,IELTS_STORE_NAMES.testRuns].includes(row.store))).map(row=>row.owner==='ielts'?{...row,databaseVersion:3}:row);return legacy;};
...
test('every durable store sentinel reaches canonical vNext payload while caches and capabilities stay out',async()=>{
  ...
  for(const store of Object.values(IELTS_STORE_NAMES)){
    if(store===IELTS_STORE_NAMES.objectiveInventory){await ielts.saveIeltsObjectiveInventoryItem(inventoryRow('sentinel'));continue;}
    if(store===IELTS_STORE_NAMES.learnerArtifacts){await ielts.autosaveLearnerTextArtifact({text:'IELTS productive sentinel',at:10});continue;}
    if(store===IELTS_STORE_NAMES.testBlueprints){await ielts.saveIeltsTestBlueprint({kind:'ielts-test-blueprint',schemaVersion:1,id:'ielts-blueprint:sentinel-1',track:'academic',hierarchyLevel:'FULL_MOCK',title:'Academic Mock 1',status:'verified',timing:{mode:'timed',totalMinutes:160},sections:[],createdAt:10,updatedAt:10});continue;}
    if(store===IELTS_STORE_NAMES.testRuns){await ielts.saveIeltsTestRun({id:'ielts-run:sentinel-1',blueprintId:'ielts-blueprint:sentinel-1',track:'academic',status:'active',startedAt:10,updatedAt:10,checkpoint:{elapsedSeconds:0,currentSectionIndex:0,currentItemIndex:0,partialResponses:{},updatedAt:10},affectsSchedule:false,evidenceEligible:false});continue;}
    const keyPath=store===IELTS_STORE_NAMES.settings?'key':'id';await ielts.__testing.putOne(store,{[keyPath]:`ielts-${store}-sentinel`,text:'IELTS sentinel'});
  }
  ...
  for(const store of Object.values(IELTS_STORE_NAMES))assert.ok(first.domains.ielts.stores[store].some(row=>store===IELTS_STORE_NAMES.objectiveInventory?row.itemId==='inventory-sentinel':store===IELTS_STORE_NAMES.learnerArtifacts?row.kind==='learner-text-artifact':(row.key??row.id)===`ielts-${store}-sentinel`||row.id==='ielts-blueprint:sentinel-1'||row.id==='ielts-run:sentinel-1'),store);
...
test('backup registry v6 includes the IELTS W0 durable stores',()=>{assert.equal(FULL_BACKUP_VERSION,6);assert.equal(BACKUP_REGISTRY_VERSION,6);});
test('registry-v4 accepts only the complete v7 historical registry and upgrades an empty private owner',()=>{const legacy=legacyIeltsV3(clone(acceptedBackup));legacy.schemaVersion=4;legacy.registryVersion=4;legacy.domains.v10.databaseVersion=7;delete legacy.domains.v10.stores[V10_STORES.privateSources];legacy.manifest.stores=legacy.manifest.stores.filter(row=>!(row.owner==='v10'&&row.store===V10_STORES.privateSources)).map(row=>row.owner==='v10'?{...row,databaseVersion:7}:row);for(const row of legacy.manifest.stores){const rows=row.backupRule==='exclude'?[]:legacy.domains[row.owner].stores[row.store];row.recordCount=rows.length;row.contentDigest=row.backupRule==='exclude'?null:`sha256:${registry.sha256Hex(JSON.stringify(rows))}`;}legacy.payloadDigest=registry.canonicalBackupDigest(legacy.domains);const valid=registry.validateFullBackupEnvelope(legacy);assert.equal(valid.valid,true);assert.deepEqual(valid.value.domains.v10.stores[V10_STORES.privateSources],[]);const injected=clone(legacy);injected.domains.v10.stores[V10_STORES.privateSources]=[];injected.payloadDigest=registry.canonicalBackupDigest(injected.domains);assert.equal(registry.validateFullBackupEnvelope(injected).valid,false);});
test('registry-v5 accepts only the complete v3 historical IELTS registry and upgrades empty test blueprints and runs',()=>{const legacy=clone(acceptedBackup);legacy.schemaVersion=5;legacy.registryVersion=5;legacy.domains.ielts.databaseVersion=3;delete legacy.domains.ielts.stores[IELTS_STORE_NAMES.testBlueprints];delete legacy.domains.ielts.stores[IELTS_STORE_NAMES.testRuns];legacy.manifest.stores=legacy.manifest.stores.filter(row=>!(row.owner==='ielts'&&[IELTS_STORE_NAMES.testBlueprints,IELTS_STORE_NAMES.testRuns].includes(row.store))).map(row=>row.owner==='ielts'?{...row,databaseVersion:3}:row);for(const row of legacy.manifest.stores){const rows=row.backupRule==='exclude'?[]:legacy.domains[row.owner].stores[row.store];row.recordCount=rows.length;row.contentDigest=row.backupRule==='exclude'?null:`sha256:${registry.sha256Hex(JSON.stringify(rows))}`;}legacy.payloadDigest=registry.canonicalBackupDigest(legacy.domains);const valid=registry.validateFullBackupEnvelope(legacy);assert.equal(valid.valid,true);assert.deepEqual(valid.value.domains.ielts.stores[IELTS_STORE_NAMES.testBlueprints],[]);assert.deepEqual(valid.value.domains.ielts.stores[IELTS_STORE_NAMES.testRuns],[]);const injected=clone(legacy);injected.domains.ielts.stores[IELTS_STORE_NAMES.testBlueprints]=[];injected.payloadDigest=registry.canonicalBackupDigest(injected.domains);assert.equal(registry.validateFullBackupEnvelope(injected).valid,false);});
test('historical schema-v2 and registry v2/v3 use exact V10 v7 store sets and reject private-source injection',()=>{
  const v2=legacyIeltsV3(legacyV10V7(clone(acceptedBackup)));v2.schemaVersion=2;v2.registryVersion=1;delete v2.manifest;v2.payloadDigest=registry.canonicalBackupDigest(v2.domains);assert.equal(registry.validateFullBackupEnvelope(v2).valid,true);
