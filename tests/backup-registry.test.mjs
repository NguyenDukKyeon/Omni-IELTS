...
const legacyIeltsV3=legacy=>{
  legacy.domains.ielts.databaseVersion=3;
  if(legacy.domains.ielts.stores.ieltsTestBlueprints)delete legacy.domains.ielts.stores.ieltsTestBlueprints;
  if(legacy.domains.ielts.stores.ieltsTestRuns)delete legacy.domains.ielts.stores.ieltsTestRuns;
  legacy.manifest.stores=legacy.manifest.stores.filter(row=>!(row.owner==='ielts'&&['ieltsTestBlueprints','ieltsTestRuns'].includes(row.store))).map(row=>row.owner==='ielts'?{...row,databaseVersion:3}:row);
  return legacy;
};
...
const registryV4=()=>{const legacy=legacyIeltsV3(legacyV10V7(clone(acceptedBackup)));legacy.schemaVersion=4;legacy.registryVersion=4;return refreshLegacy(legacy);};
const registryV5=()=>{const legacy=legacyIeltsV3(clone(acceptedBackup));legacy.schemaVersion=5;legacy.registryVersion=5;return refreshLegacy(legacy);};
...
test('every durable store sentinel reaches canonical vNext payload while caches and capabilities stay out',async()=>{
  ...
  for(const store of Object.values(IELTS_STORE_NAMES)){
    ...
    if(store===IELTS_STORE_NAMES.testBlueprints||store==='ieltsTestBlueprints'){
      await ielts.saveIeltsTestBlueprint({
        kind:'ielts-test-blueprint',schemaVersion:1,id:'ielts-blueprint:sentinel-1',track:'academic',hierarchyLevel:'FULL_MOCK',
        title:'Sentinel Mock',description:'Sentinel',status:'verified',timingPolicy:{mode:'exam',totalMinutes:160},
        interruptionPolicy:{taskFamily:'RESTART_EXISTING_RUN',sectionMock:'CHECKPOINT_AND_RESUME'},sections:[{kind:'ielts-section-blueprint',schemaVersion:1,sectionId:'s1',skill:'reading',order:1,timeLimitMinutes:60,partCount:3,itemCount:40,sourceRevisionRef:{schema:'SourceRevisionRef',version:1,kind:'private-pack',authority:'test',sourceId:'s1',revisionId:'r1',integrity:'0'.repeat(64),locator:{assetId:'a1'},provenance:{origin:'test',verification:'verified',rights:'allowed',privacy:'private'}}}],
        createdAt:'2026-08-15T00:00:00.000Z',updatedAt:'2026-08-15T00:00:00.000Z'
      });
      continue;
    }
    if(store===IELTS_STORE_NAMES.testRuns||store==='ieltsTestRuns'){
      await ielts.saveIeltsTestRun({
        kind:'ielts-test-run',schemaVersion:1,id:'ielts-run:sentinel-1',blueprintId:'ielts-blueprint:sentinel-1',track:'academic',
        hierarchyLevel:'FULL_MOCK',status:'active',affectsSchedule:false,evidenceEligible:false,checkpoint:{currentSectionIndex:0,elapsedSeconds:0,answers:{},savedAt:'2026-08-15T00:00:00.000Z'},
        createdAt:'2026-08-15T00:00:00.000Z',updatedAt:'2026-08-15T00:00:00.000Z'
      });
      continue;
    }
  ...
  for(const store of Object.values(IELTS_STORE_NAMES)){
    assert.ok(first.domains.ielts.stores[store].length>0,`Store ${store} must reach backup payload`);
  }
...
test('backup registry v6 includes the new IELTS durable stores',()=>{assert.equal(FULL_BACKUP_VERSION,6);assert.equal(BACKUP_REGISTRY_VERSION,6);});
test('registry-v4 accepts only the complete v7 historical registry and upgrades an empty private owner',()=>{const legacy=registryV4();const valid=registry.validateFullBackupEnvelope(legacy);assert.equal(valid.valid,true);assert.deepEqual(valid.value.domains.v10.stores[V10_STORES.privateSources],[]);const injected=clone(legacy);injected.domains.v10.stores[V10_STORES.privateSources]=[];injected.payloadDigest=registry.canonicalBackupDigest(injected.domains);assert.equal(registry.validateFullBackupEnvelope(injected).valid,false);});
test('registry-v5 accepts the authentic v5 predecessor and upgrades empty test blueprints and runs',()=>{const legacy=registryV5();const valid=registry.validateFullBackupEnvelope(legacy);assert.equal(valid.valid,true);assert.deepEqual(valid.value.domains.ielts.stores[IELTS_STORE_NAMES.testBlueprints||'ieltsTestBlueprints'],[]);assert.deepEqual(valid.value.domains.ielts.stores[IELTS_STORE_NAMES.testRuns||'ieltsTestRuns'],[]);});
