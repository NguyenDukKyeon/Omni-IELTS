...
// ============================================================================
// AH. (F001) Exact Activation State Validation
// ============================================================================
test('AH (F001): non-ready activation states fail closed with ACTIVATION_RECORD_INVALID', async () => {
  const repo = createTempGitRepo();
  try {
    const authDir = path.join(repo.dir, 'docs', 'authorizations');
    fs.mkdirSync(authDir, { recursive: true });
    ...
    for (const state of invalidStates) {
      const statusContent = [
        '# Implementation Status',
        '',
        makeV1ActivationSection('TEST-TX-001', 'docs/authorizations/TEST-AUTH-001.md', 'TEST-AUTH-001', acceptedHead, 'PRR_TEST_001', { state }),
      ].join('\n');

      fs.writeFileSync(path.join(repo.dir, 'docs', 'IMPLEMENTATION_STATUS.md'), statusContent);
      repo.git(['add', '.']);
      repo.git(['commit', '-m', `activation commit with state ${state}`]);
      repo.git(['update-ref', 'refs/remotes/origin/main', 'HEAD']);

      const result = await compileContext('TEST-TX-001', { rootDir: repo.dir });
      assert.equal(result.ok, false, `State "${state}" must not emit an authorized capsule`);
      assert.equal(result.error, 'ACTIVATION_RECORD_INVALID', `State "${state}" must fail with ACTIVATION_RECORD_INVALID`);
    }
  } finally {
    repo.cleanup();
  }
...
// ============================================================================
// AI. (F002) Self-Resolve Fields Completeness & Exact Value Enforcement
// ============================================================================
test('AI (F002): missing or non-self-resolving markers fail closed with ACTIVATION_RECORD_INVALID', async () => {
  const repo = createTempGitRepo();
  try {
    const authDir = path.join(repo.dir, 'docs', 'authorizations');
    fs.mkdirSync(authDir, { recursive: true });
    ...
    // 1. Missing Authorization Merge SHA
    const sectionWithoutMergeSha = [
      '## AGENT_CONTEXT_AUTH_ACTIVATION_V1 — TEST-TX-001',
      '',
      '| Field | Value |',
      '|---|---|',
      ...
      '| Authorization Manifest | `docs/authorizations/TEST-AUTH-001.md` |',
      '| Authorization Manifest Identity | `TEST-AUTH-001` |',
      `| Authorization Accepted Head | \`${acceptedHead}\` |`,
      '| Independent Authorization Review | `PRR_TEST_001` |',
      '| Activation State | `AUTHORIZED / READY_FOR_EXECUTION` |',
      '| Effective Implementation Predecessor | `SELF_RESOLVE_ACTIVATION_MERGE_SHA` |',
    ].join('\n');

    fs.writeFileSync(path.join(repo.dir, 'docs', 'IMPLEMENTATION_STATUS.md'), `# Status\n\n${sectionWithoutMergeSha}\n`);
    repo.git(['add', '.']);
    repo.git(['commit', '-m', 'missing merge sha']);
    repo.git(['update-ref', 'refs/remotes/origin/main', 'HEAD']);

    let result = await compileContext('TEST-TX-001', { rootDir: repo.dir });
    assert.equal(result.ok, false);
    assert.equal(result.error, 'ACTIVATION_RECORD_INVALID');

    // 2. Missing Effective Implementation Predecessor
    const sectionWithoutPredecessor = [
      '## AGENT_CONTEXT_AUTH_ACTIVATION_V1 — TEST-TX-001',
      '',
      '| Field | Value |',
      '|---|---|',
    ...
    fs.writeFileSync(path.join(repo.dir, 'docs', 'IMPLEMENTATION_STATUS.md'), `# Status\n\n${sectionWithoutPredecessor}\n`);
    repo.git(['add', '.']);
    repo.git(['commit', '-m', 'missing predecessor']);
    repo.git(['update-ref', 'refs/remotes/origin/main', 'HEAD']);

    result = await compileContext('TEST-TX-001', { rootDir: repo.dir });
    assert.equal(result.ok, false);
    assert.equal(result.error, 'ACTIVATION_RECORD_INVALID');

    // 3. Arbitrary SHA / text in Authorization Merge SHA
    const sectionWithArbitraryMergeSha = makeV1ActivationSection('TEST-TX-001', 'docs/authorizations/TEST-AUTH-001.md', 'TEST-AUTH-001', acceptedHead, 'PRR_TEST_001', {
      authMergeSha: '4937fc3d62ab80bbed651409882d3d3a20735bb6',
    });
    fs.writeFileSync(path.join(repo.dir, 'docs', 'IMPLEMENTATION_STATUS.md'), `# Status\n\n${sectionWithArbitraryMergeSha}\n`);
    repo.git(['add', '.']);
    repo.git(['commit', '-m', 'arbitrary merge sha']);
    repo.git(['update-ref', 'refs/remotes/origin/main', 'HEAD']);
    ...
    // 4. Arbitrary text in Effective Implementation Predecessor
    const sectionWithArbitraryPredecessor = makeV1ActivationSection('TEST-TX-001', 'docs/authorizations/TEST-AUTH-001.md', 'TEST-AUTH-001', acceptedHead, 'PRR_TEST_001', {
      predecessor: 'CUSTOM_PREDECESSOR_VALUE',
    });
    fs.writeFileSync(path.join(repo.dir, 'docs', 'IMPLEMENTATION_STATUS.md'), `# Status\n\n${sectionWithArbitraryPredecessor}\n`);
    repo.git(['add', '.']);
    repo.git(['commit', '-m', 'arbitrary predecessor']);
    repo.git(['update-ref', 'refs/remotes/origin/main', 'HEAD']);
...
// ============================================================================
// AJ. (F003) Duplicate Field Row Ambiguity Fails Closed
// ============================================================================
test('AJ (F003): duplicate field rows in V1 activation table fail closed with ACTIVATION_RECORD_AMBIGUOUS', async () => {
  const repo = createTempGitRepo();
  try {
    const authDir = path.join(repo.dir, 'docs', 'authorizations');
    fs.mkdirSync(authDir, { recursive: true });
    ...
    const duplicateCases = [
      ['Activation State', '| Activation State | `BLOCKED` |', '| Activation State | `AUTHORIZED / READY_FOR_EXECUTION` |'],
      ['Authorization Manifest', '| Authorization Manifest | `docs/authorizations/TEST-AUTH-001.md` |', '| Authorization Manifest | `docs/authorizations/TEST-AUTH-001.md` |'],
      ['Authorization Manifest Identity', '| Authorization Manifest Identity | `TEST-AUTH-001` |', '| Authorization Manifest Identity | `TEST-AUTH-001` |'],
      ['Authorization Accepted Head', `| Authorization Accepted Head | \`${acceptedHead}\` |`, `| Authorization Accepted Head | \`${acceptedHead}\` |`],
      ['Authorization Merge SHA', '| Authorization Merge SHA | `SELF_RESOLVE_ACTIVATION_MERGE_SHA` |', '| Authorization Merge SHA | `SELF_RESOLVE_ACTIVATION_MERGE_SHA` |'],
      ['Effective Implementation Predecessor', '| Effective Implementation Predecessor | `SELF_RESOLVE_ACTIVATION_MERGE_SHA` |', '| Effective Implementation Predecessor | `SELF_RESOLVE_ACTIVATION_MERGE_SHA` |'],
      ['Independent Authorization Review', '| Independent Authorization Review | `PRR_001` |', '| Independent Authorization Review | `PRR_002` |'],
      ['Transaction ID', '| Transaction ID | `TEST-TX-001` |', '| Transaction ID | `TEST-TX-001` |'],
    ];

    for (const [fieldName, row1, row2] of duplicateCases) {
      const baseSection = [
        '## AGENT_CONTEXT_AUTH_ACTIVATION_V1 — TEST-TX-001',
        '',
        '| Field | Value |',
      ...
      fs.writeFileSync(path.join(repo.dir, 'docs', 'IMPLEMENTATION_STATUS.md'), `# Status\n\n${baseSection}\n`);
      repo.git(['add', '.']);
      repo.git(['commit', '-m', `duplicate ${fieldName}`]);
      repo.git(['update-ref', 'refs/remotes/origin/main', 'HEAD']);

      const result = await compileContext('TEST-TX-001', { rootDir: repo.dir });
      assert.equal(result.ok, false, `Duplicate field ${fieldName} must fail closed`);
      assert.equal(result.error, 'ACTIVATION_RECORD_AMBIGUOUS', `Duplicate field ${fieldName} must fail with ACTIVATION_RECORD_AMBIGUOUS`);
    }
  } finally {
    repo.cleanup();
  }
...
// ============================================================================
// AK. (F004) Canonical Authorization Provenance Blob SHA Binding
// ============================================================================
test('AK (F004): canonical authorization provenance blob SHA binds to commit M, not working tree', async () => {
  const repo = createTempGitRepo();
  try {
    const authDir = path.join(repo.dir, 'docs', 'authorizations');
    fs.mkdirSync(authDir, { recursive: true });
    ...
    const canonicalAuthBlob = repo.git(['rev-parse', 'HEAD:docs/authorizations/TEST-AUTH-001.md']);

    // Mutate the working tree copy of the authorization manifest
    fs.appendFileSync(path.join(authDir, 'TEST-AUTH-001.md'), '\n\nTAMPERED WORKING TREE CONTENT\n');
    const tamperedAuthBlob = repo.git(['hash-object', path.join(authDir, 'TEST-AUTH-001.md')]);

    assert.notEqual(canonicalAuthBlob, tamperedAuthBlob);

    const result = await compileContext('TEST-TX-001', { rootDir: repo.dir });
    assert.equal(result.ok, true);

    const authRef = result.capsule.authority_ledger_references.find(r => r.source_path === 'docs/authorizations/TEST-AUTH-001.md');
    assert.ok(authRef, 'Must contain authority ledger reference for manifest');
    assert.equal(authRef.source_blob_sha, canonicalAuthBlob, 'Authority ledger provenance blob SHA must match canonical Git blob at M');
    assert.notEqual(authRef.source_blob_sha, tamperedAuthBlob, 'Authority ledger provenance blob SHA must NOT match tampered working tree');

    assert.equal(result.capsule.authorization_manifest_status.provenance.source_blob_sha, canonicalAuthBlob);
  } finally {
    repo.cleanup();
  }
});
...
// ============================================================================
// AL. (F005) Canonical Status Provenance Blob SHA Binding
// ============================================================================
test('AL (F005): canonical status provenance blob SHA binds to commit M, not working tree', async () => {
  const repo = createTempGitRepo();
  try {
    const authDir = path.join(repo.dir, 'docs', 'authorizations');
    fs.mkdirSync(authDir, { recursive: true });
    ...
    const canonicalStatusBlob = repo.git(['rev-parse', 'HEAD:docs/IMPLEMENTATION_STATUS.md']);

    // Mutate the working tree copy of IMPLEMENTATION_STATUS.md
    fs.appendFileSync(path.join(repo.dir, 'docs', 'IMPLEMENTATION_STATUS.md'), '\n\nTAMPERED WORKING TREE STATUS\n');
    const tamperedStatusBlob = repo.git(['hash-object', path.join(repo.dir, 'docs', 'IMPLEMENTATION_STATUS.md')]);

    assert.notEqual(canonicalStatusBlob, tamperedStatusBlob);

    const result = await compileContext('TEST-TX-001', { rootDir: repo.dir });
    assert.equal(result.ok, true);

    const statusRef = result.capsule.authority_ledger_references.find(r => r.source_path === 'docs/IMPLEMENTATION_STATUS.md');
    assert.ok(statusRef, 'Must contain authority ledger reference for status');
    assert.equal(statusRef.source_blob_sha, canonicalStatusBlob, 'Status ledger provenance blob SHA must match canonical Git blob at M');
    assert.notEqual(statusRef.source_blob_sha, tamperedStatusBlob, 'Status ledger provenance blob SHA must NOT match tampered working tree');

    assert.equal(result.capsule.current_execution_status.provenance.source_blob_sha, canonicalStatusBlob);
    assert.equal(result.capsule.transaction_predecessor.provenance.source_blob_sha, canonicalStatusBlob);
  } finally {
    repo.cleanup();
  }
});
