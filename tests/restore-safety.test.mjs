...
async function seedSentinels(label){
  for(const entry of registry.BACKUP_STORE_REGISTRY.filter(row=>row.backupRule!=='exclude')){
    ...
    else if(entry.owner==='ielts'&&(entry.store===IELTS_STORE_NAMES.testBlueprints||entry.store==='ieltsTestBlueprints'))await ielts.saveIeltsTestBlueprint({kind:'ielts-test-blueprint',schemaVersion:1,id:`ielts-blueprint:${label}`,track:'academic',hierarchyLevel:'FULL_MOCK',title:`${label} blueprint`,description:'test',status:'verified',timingPolicy:{mode:'exam',totalMinutes:160},interruptionPolicy:{taskFamily:'RESTART_EXISTING_RUN',sectionMock:'CHECKPOINT_AND_RESUME'},sections:[{kind:'ielts-section-blueprint',schemaVersion:1,sectionId:`${label}-s1`,skill:'reading',order:1,timeLimitMinutes:60,partCount:3,itemCount:40,sourceRevisionRef:{schema:'SourceRevisionRef',version:1,kind:'private-pack',authority:'test',sourceId:'s1',revisionId:'r1',integrity:'0'.repeat(64),locator:{assetId:'a1'},provenance:{origin:'test',verification:'verified',rights:'allowed',privacy:'private'}}}],createdAt:'2026-08-15T00:00:00.000Z',updatedAt:'2026-08-15T00:00:00.000Z'});
    else if(entry.owner==='ielts'&&(entry.store===IELTS_STORE_NAMES.testRuns||entry.store==='ieltsTestRuns'))await ielts.saveIeltsTestRun({kind:'ielts-test-run',schemaVersion:1,id:`ielts-run:${label}`,blueprintId:`ielts-blueprint:${label}`,track:'academic',hierarchyLevel:'FULL_MOCK',status:'active',affectsSchedule:false,evidenceEligible:false,checkpoint:{currentSectionIndex:0,elapsedSeconds:10,answers:{},savedAt:'2026-08-15T00:00:00.000Z'},createdAt:'2026-08-15T00:00:00.000Z',updatedAt:'2026-08-15T00:00:00.000Z'});
