...
test('Core, IELTS and V10 adopt one durable migration ledger entry and double-open is idempotent',async()=>{
  ...
  try{
    ...
    assert.deepEqual(ledgers.map(rows=>rows.map(row=>row.migrationId)),[
      ...
      ['p1-00-ielts-opener-v1','wave4-ielts-profile-inventory-v2','wave5-productive-text-artifacts-v3','wave0-ielts-product-contracts-v4'],
    ...
    assert.deepEqual(ledgers[1].map(row=>({migrationId:row.migrationId,targetVersion:row.targetVersion,digest:row.digest,mode:row.mode})),[
      ...
      {migrationId:'wave0-ielts-product-contracts-v4',targetVersion:4,digest:'wave0-ielts-product-contracts-store-v4:2026-08-15',mode:'upgrade'}
    ]);
    ...
    assert.deepEqual(second.map(rows=>rows.length),[2,4,8]);
...
test('physical IELTS v1 upgrade preserves every legacy sentinel and additively creates objective inventory and learner artifacts',async()=>{
  ...
  globalThis.indexedDB=factory;
  const legacy=Object.values(IELTS_STORE_NAMES).filter(store=>![IELTS_STORE_NAMES.objectiveInventory,IELTS_STORE_NAMES.learnerArtifacts,IELTS_STORE_NAMES.frozenAssessments,IELTS_STORE_NAMES.testBlueprints,IELTS_STORE_NAMES.testRuns].includes(store));
