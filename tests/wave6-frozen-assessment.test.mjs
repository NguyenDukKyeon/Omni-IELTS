...
test('existing QAR and IELTS prerequisites are healthy before Frozen Assessment capability is required',async()=>{
  ...
  registry.registerExecutor(question.kind,question.version,question.requiredCapabilities);
  assert.equal(registry.supports(question),true,'existing QAR registry must support the authentic prerequisite question');
  assert.equal(registry.hasExecutor(question),true,'existing QAR registry prerequisite must be healthy');
