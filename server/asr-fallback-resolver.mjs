...
export function createAsrFallbackResolver({repository,localClient,cloudClient,securityHeaders}={}){
  ...
  const runCloud=async jobId=>{
    const slot=`cloud:${jobId}`;if(workers.has(slot))return workers.get(slot).promise;let job=await repository.get(jobId);if(!job)throw resolverError('UNKNOWN','Resolver job not found.');
    if(job.status==='failed')await repository.transition(jobId,'queued',{error:null});const claim=await repository.claimForWorker(jobId,`${owner}:cloud:${Date.now()}`);if(!claim.claimed)return claim.job;
    const controller=new AbortController(),work=(async()=>{try{job=await repository.get(jobId);const health=await cloudClient.health();if(!health.available)throw resolverError('CLOUD_UNAVAILABLE','Gemini is not configured on the server.');const result=await cloudClient.transcribe(job,{signal:controller.signal}),sentences=mergeAsrBatches([result.segments||[]]);return(await repository.transition(jobId,'complete',{provider:'gemini-progressive',sentences,metadata:{title:'Gemini transcript',durationSeconds:result.durationSeconds,track:{kind:'gemini',language:job.request.language}},coverage:{coveredMs:Math.max(0,...sentences.map(row=>row.endMs)),startMs:sentences[0]?.startMs??null,endMs:sentences.at(-1)?.endMs??null,complete:true,gaps:[]},provenance:{private:true,verified:false,needsReview:true,shared:false,uploadedFileRetained:false,billableRequests:result.billableRequests}})).job;}catch(error){const latest=await repository.get(jobId);if(latest&&!['cancelled','complete','failed'].includes(latest.status))await repository.transition(jobId,'failed',{error:safeError(error)});throw error;}finally{workers.delete(slot);}})();workers.set(slot,{controller,promise:work});return work;
  };
  return{
