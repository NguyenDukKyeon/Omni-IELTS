...
**Remediation Candidate: STAGE3-R3-PIPELINE-ARCHITECTURE-RESEARCH-REM-004**

- **Document Identifier**: `R3_PIPELINE_ARCHITECTURE_RESEARCH.md`
- **Transaction Identity**: `STAGE3-R3-PIPELINE-ARCHITECTURE-RESEARCH-REM-004`
- **Historical Subjects / Superseded Candidates**: PR #160 (`c3793931c29eaeb85ecad3b38622f19c1da2cd53`), PR #161 (`56616336f2e6dbaee5f16b2e6eb15a395371ffaf`), PR #162 (`da2b677d1a10893235b6d3c6c7e3fd4257f61928`), PR #163 (`4b26097bc64537c4dc1ce58d35dd85bb26b9e6c1`)
...
| **R3-F004** | `[VERIFIED]` | Execution / Streaming | Current transcript resolver operates via server-side polling / SSE job transitions (`queued` $\to$ `resolving` $\to$ `partial` $\to$ `complete`), but client-side learning derivation and workspace mounting parse and clone entire transcripts as batch arrays rather than streaming pipelines. | `openVideoWorkspace`, `ResolverJob` (`src/video-workspace-v2.js`, `src/resolver-contracts.js`) |
...
| **R3-F010** | `[VERIFIED]` | Privacy / ASR | Local ASR is isolated to a desktop loopback companion process (`127.0.0.1`); mobile devices never advertise local companion and rely on public captions, private file import rescue, or consented cloud fallback. | `server/caption-resolver-v2.mjs`, `src/transcript-resolver-v2.js` |
| **R3-F011** | `[VERIFIED]` | Privacy / Consent | Cloud ASR fallback (Gemini) requires an explicit, versioned, replay-safe, and revocable consent receipt (`phase5-gemini-consent-v1`) stored in durable metadata before any external transmission. | `phase5-gemini-consent-v1`, `src/transcript-resolver-v2.js` |
| **R3-F012** | `[VERIFIED]` | Audio / Manager | `audio-manager.js` encapsulates Web Speech API speech synthesis and Web Audio MediaRecorder with deterministic mock fallbacks for headless CI; audio recording blobs are not durably persisted in IndexedDB. | `src/audio-manager.js`, `tests/audio-manager.test.mjs` |
| **R3-F013** | `[VERIFIED]` | Learner Model / FSRS | Memory scheduling is implemented via `ts-fsrs` (v5.4.1 / FSRS v6) with 5 skill dimensions (`recognition`, `recall`, `listening`, `collocation`, `production`), operating independently of latent mastery or psychometric item difficulty calibration. | `FSRS_VERSION`, `applyFsrsRating`, `LEARNING_SKILLS` (`src/fsrs-scheduler.js`, `src/schedule-gateway.js`, `src/learning-contracts.js`) |
| **R3-F014** | `[VERIFIED]` | Learner Model / Weakness | `weakness-profile.js` implements deterministic error projection from canonical review events and error records across 23 categories; it explicitly reports sample size, denominator, uncertainty, and insufficient-data states rather than guessing. | `src/weakness-profile.js`, `src/ielts-domain.js` |
| **R3-F015** | `[VERIFIED]` | Today / Orchestration | `today-runner.js` leases a single active run across browser tabs and requires immutable target re-validation (`ActivitySpec`) before execution, failing closed on stale or missing targets. | `src/today-runner.js`, `src/storage-lock.js` |
| **R3-F016** | `[INFERENCE]` | Pipeline Architecture | Decoupling the monolithic transcript pipeline into distinct asynchronous stages (Acquisition $\to$ Normalization $\to$ SBD/Punctuation $\to$ Timestamp Alignment $\to$ Linguistic Enrichment $\to$ Synthesis) is necessary to enable progressive processing semantics and reduce Main Thread contention. | Architectural deduction from `src/caption-normalizer.js` & `src/video-workspace-v2.js` |
...
| **R3-F004** | `[VERIFIED]` | `src/video-workspace-v2.js`: `openVideoWorkspace`, `src/resolver-contracts.js` | `tests/phase3-video-workspace.test.mjs`, `tests/video-workspace-static.test.mjs` | W3C Fetch / SSE Specs | `STATIC_CODE_VERIFIED` | Workspace consumes full transcript array in memory at mount time. |
...
| **R3-F008** | `[VERIFIED]` | `src/video-workspace-v2.js`, `src/caption-normalizer.js`, `structuredClone` | `tests/phase3-video-workspace.test.mjs`, `tests/video-workspace-static.test.mjs` | HTML Living Standard (Event Loop) | `NOT_EMPIRICALLY_MEASURED` (`STAGE5_MEASUREMENT_REQUIRED`) | Synchronous execution on main UI thread; potential main-thread contention risk. |
...
| **R3-F013** | `[VERIFIED]` | `src/fsrs-scheduler.js`: `FSRS_VERSION`, `applyFsrsRating`, `src/schedule-gateway.js`: `commitCoreEvidence`, `src/learning-contracts.js`: `LEARNING_SKILLS` | `tests/learning.test.mjs`, `tests/schedule-gateway.test.mjs`, `tests/v10-evidence.test.mjs` | FSRS v6 / `ts-fsrs` (v5.4.1) | `TEST_ASSERTION_VERIFIED` | 5 skills supported; operates independently of IRT difficulty or BKT mastery. |
...
| **R3-F016** | `[INFERENCE]` | `src/caption-normalizer.js`, `src/video-workspace-v2.js` | N/A (Architectural Deduction) | Reactive Streams / Pipeline Patterns | `NOT_EMPIRICALLY_MEASURED` (`STAGE5_MEASUREMENT_REQUIRED`) | Pipeline decoupling reduces Main Thread contention risk subject to overhead. |
...
| **R3-F019** | `[INFERENCE]` | `src/transcript-resolver-v2.js`, `src/video-workspace-v2.js` | N/A (Streaming Deduction) | Stream Processing Architecture | `NOT_EMPIRICALLY_MEASURED` (`STAGE5_MEASUREMENT_REQUIRED`) | Chunked processing enables progressive learning access on long media. |
...
│ │ - YouTube Sentence Player│  │ - Exact Target Leaser    │  │ - Quick Capture Form    │ │
...
│ │ │ - Immutable Revision │  │ - Schedule Gateway   │  │ - Weakness Target Selector  │ │ │
│ │ └──────────────────────┘  └──────────────────────┘  └─────────────────────────────┘ │ │
│ └──────────────────────────────────────────┬──────────────────────────────────────────┘ │
│                                            │                                            │
│ ┌──────────────────────────────────────────▼──────────────────────────────────────────┐ │
...
- **State Ownership**: In-memory job state, filesystem JSON backend (`resolver-jobs-v2.json` via `server/resolver-job-repository.mjs`), and IndexedDB store `V10_STORES.resolverJobs` (`src/v10-contracts.js`).
...
- **Actual Paths**: `src/caption-normalizer.js`, `src/transcript-aggregate.js`, `src/source-revision-ref.js`, `src/transcript-import.js`.
- **Current State**: `CURRENTLY_IMPLEMENTED`.
...
- **Actual Paths**: `src/video-workspace-v2.js`, `src/youtube-sentence-player.js`, `src/ielts-media-player.js`.
- **Current State**: `CURRENTLY_IMPLEMENTED`.
- **Responsibility**: Orchestrate YouTube IFrame player synchronization (`createYoutubeSentencePlayer`, `YouTubeSegmentPlayer`), active sentence cue tracking (`findActiveSentenceIndex`), virtual window rail (`virtualWindow`), loop playback (`VocabMasterSentenceLoop`), and dictation/shadowing input events.
- **Input**: `row` `{ videoId, transcriptRevisionId, segments, durationSeconds, complete, ... }`, `{ startIndex, mode }`.
- **Output**: Interactive DOM workspace with synchronized time updates and exercise event dispatches (`openVideoWorkspace`, `mountVideoWorkspaceV2`).
- **State Ownership**: DOM state, player state machine (`YouTubeSegmentPlayer.playerState`), and playback position.
- **Sync/Async**: Asynchronous bridge events coupled with time polling.
- **Persistence Boundary**: Ephemeral UI state; user interactions dispatched to `EvidencePolicy` and `Attempt` storage.
- **Failure Behavior**: Graceful playback error handling (`TRANSCRIPT_UNALIGNED`, `TRANSCRIPT_INELIGIBLE`, player error callbacks).

#### `COMP-04`: Learning Contracts & Activity Specification (`P1-02`, `P1-08`, `P4-00`, `P7-00`)
- **Actual Paths**: `src/learning-contracts.js`, `src/question-activity-contracts.js`, `src/productive-text-contracts.js`, `src/content-contracts-v2.js`.
- **Current State**: `CURRENTLY_IMPLEMENTED`.
- **Responsibility**: Define immutable, frozen domain contracts for learning operations: `ActivitySpec`, `Run`, `Attempt`, `Receipt`, `AssistanceTrace`, and `EvidenceDecision`, with strict runtime validation (`validateActivitySpec`, `validateRun`, `validateAttempt`, `validateReceipt`, `validateLearningEnvelope`, `validateFrozenRunBinding`).
...
- **Persistence Boundary**: `Core` DB (`learningEvents`, `learningProjections`), `IELTS` DB (`readingAttempts`, `mediaAttempts`, `ieltsTestRuns`), and `V10` DB (`todayRuns`, `activities`).
...
- **Current State**: `CURRENTLY_IMPLEMENTED`.
- **Responsibility**: Sole write gateway transforming learner attempts into qualified review events. Evaluates assistance exposure, source authority, and evaluation validity (`decideEvidence`, `normalizeAssistanceTrace`).
...
- **Actual Paths**: `src/fsrs-scheduler.js`, `src/schedule-gateway.js`, `src/persistence.js`, `src/today-runner.js`.
- **Current State**: `CURRENTLY_IMPLEMENTED`.
- **Responsibility**: Spaced repetition interval and stability calculations via `ts-fsrs` (v5.4.1 / FSRS v6, `FSRS_VERSION = 6`). Gateway pipeline `commitCoreEvidence` coordinates evidence qualification with FSRS parameter updates (`applyFsrsRating`) and transactional writes (`persistReviewResult`) to Core DB `cards` and `reviewEvents` stores. Orchestrates daily Today queue leasing and target resolution (`today-runner.js`).
- **Input**: `card` state `{ stability, difficulty, elapsed_days, scheduled_days, reps, lapses, state, last_review, fsrsBySkill }`, `rating` (`again`|`hard`|`good`|`easy`), `now` timestamp, `skill`.
- **Output**: Updated `card` record and scheduled due dates.
- **State Ownership**: `Core` DB `cards` and `reviewEvents`, and `V10` DB `todayRuns`.
- **Sync/Async**: Asynchronous persistence writes protected by Web Locks storage coordinator (`storage-lock.js`).
...
- **State Ownership**: Derived projection cache and durable error stores (`IELTS_STORE_NAMES.errorRecords`, `V10_STORES.globalErrorRecords`, `repairQueue`).
...
- **Actual Paths**: `src/persistence.js`, `src/ielts-persistence.js`, `src/v10-persistence.js`, `src/storage-lock.js`, `src/backup-registry.js`, `src/persistence-core.js`, `src/migration-ledger.js`.
- **Current State**: `CURRENTLY_IMPLEMENTED`.
...
- **Current Substrate & Proven Evidence Linkage**:
  - **Core Card FSRS Decoupling**: In `src/learning-contracts.js`, `src/evidence-policy.js`, `src/schedule-gateway.js`, and `src/fsrs-scheduler.js`, `EvidencePolicy` evaluates attempts targeting `core-card` entities (`cardId`), emitting `reviewEvents` keyed by `cardId` and `skill` via `commitCoreEvidence`. Core FSRS memory scheduling is bound strictly to vocabulary card IDs and does **NOT** bind directly to transcript segment IDs. Resegmenting a transcript or modifying cue timestamps does **NOT** orphan or alter core card FSRS progress.
...
| **2. Source Occurrence Identity** | `CURRENTLY_IMPLEMENTED_DIMENSION` (Coupled to timestamps in `src/caption-normalizer.js`) | Explicit ordinal index ($k$-th occurrence in source $S$). | The specific $k$-th chronological occurrence of a sentence in source $S$. | `sourceId` + `sourceOrdinalIndex` + `semanticContentKey`. | **Medium**: Stable unless upstream sentences are added/deleted. |
...
| **5. Learner Target Lineage** | `CURRENTLY_IMPLEMENTED_DIMENSION` for Core Cards (`cardId` in `src/learning-contracts.js`); `CURRENTLY_MISSING_DIMENSION` for transcript revision translation | Cross-revision translation mapping bridging revised segments to historical sentence practice. | Durable anchor linking learner practice records to target constructs across revisions. | `targetType` + `canonicalTargetKey` + `lineageMappingRef`. | **Permanent**: Core card FSRS survives transcript changes; sentence practice reconciled via mapping tables. |
...
|---|---|---|
| **Session Review Ratio** | Hardcoding fixed 60–70% review vs 30–40% new-learning allocations. | Session orchestrator (`src/today-runner.js`) accepts a configurable queue policy `{ reviewAllocationRatio, maxNewCards, dynamicBacklogCap }`. |
...
| **Statistical Variance Modeling** | Hardcoding arbitrary 3-attempt failure thresholds for skill regression. | Error tracking reducer (`src/weakness-profile.js`) accepts configurable statistical control parameters (CUSUM / Shewhart bounds). |
...
- **CURRENT_STATE**: `CURRENT_GAP`
- **RAW_EVIDENCE**: `src/video-workspace-v2.js`, `src/caption-normalizer.js`.
...
- **CURRENT_STATE**: `CURRENT_GAP`
- **RAW_EVIDENCE**: `src/transcript-resolver-v2.js`, `src/video-workspace-v2.js`.
...
- **CURRENT_STATE**: `CURRENT_GAP`
- **RAW_EVIDENCE**: `src/persistence.js`, `src/schedule-gateway.js`.
...
- **CURRENT_STATE**: `CURRENT_GAP`
- **RAW_EVIDENCE**: `src/fsrs-scheduler.js`, `src/schedule-gateway.js`, `src/learning-contracts.js`.
...
|---|---|---|
| `CURRENT_DUPLICATE_SENTENCE_COLLISION_CLAIMS` | **PASSED (0 Claims)** | Verified `src/caption-normalizer.js` sentence identity incorporates `startMs`/`endMs` (`SAME_TEXT_AT_DIFFERENT_OCCURRENCES != SAME_SENTENCE_ID`); zero collision claims remaining. |
| `UNPROVEN_FSRS_SEGMENT_BINDING_CLAIMS` | **PASSED (0 Claims)** | Fresh verification of `src/learning-contracts.js`, `src/evidence-policy.js`, `src/schedule-gateway.js`, and `src/fsrs-scheduler.js` establishes core FSRS memory binds strictly to `cardId`, not segment IDs. |
...
| `ALL_CURRENT_SOURCE_PATHS_EXIST` | **PASSED (0 Nonexistent Paths)** | Verified all cited source, server, and test paths exist at canonical base `79cb8ef9dfcbd4493c5191af5cd9845b85784a23` with zero nonexistent files, zero nonexistent symbols, and zero stale path references. |
...
- `src/video-workspace-v2.js` — Video workspace orchestration, active sentence tracking, and practice segment preparation.
- `src/youtube-sentence-player.js` & `src/ielts-media-player.js` — YouTube IFrame player synchronization, time polling, and loop playback.
- `src/learning-contracts.js` — Immutable domain contracts (`ActivitySpec`, `Run`, `Attempt`, `Receipt`, `EvidenceDecision`, `learningContractDigest`, validation functions).
- `src/evidence-policy.js` — Default-deny evidence gateway and assistance exposure tracer (`phase0-evidence-v1`).
- `src/fsrs-scheduler.js` — FSRS memory scheduling engine (v6 / `ts-fsrs` v5.4.1), skill mapping, and retrievability calculations.
- `src/schedule-gateway.js` — Evidence qualification to FSRS scheduling orchestration and Core DB review persistence dispatch (`commitCoreEvidence`).
...
- `src/persistence.js` — Core database (`vocab-master-personal`, v5, 11 stores, 2 migrations, `persistReviewResult`).
...
- `server/caption-resolver-v2.mjs` & `server/resolver-job-repository.mjs` — Server-side caption resolution and job persistence (`resolver-jobs-v2.json`).
