...
**Remediation Candidate: STAGE3-R3-PIPELINE-ARCHITECTURE-RESEARCH-REM-001**

- **Document Identifier**: `R3_PIPELINE_ARCHITECTURE_RESEARCH.md`
- **Transaction Identity**: `STAGE3-R3-PIPELINE-ARCHITECTURE-RESEARCH-REM-001`
- **Historical Subject / Superseded Candidate**: PR #160 (`c3793931c29eaeb85ecad3b38622f19c1da2cd53`)
...
| **R3-F003** | `[VERIFIED]` | Identity / Digest | Sentence and segment IDs are derived from cryptographic hashes of exact text and timestamps (`learningContractDigest`); any algorithmic re-segmentation or timestamp shift invalidates downstream target links unless explicit lineage mapping is maintained. | `learningContractDigest` (`src/learning-contracts.js`, `src/caption-normalizer.js`) |
...
| **R3-F012** | `[VERIFIED]` | Audio / Manager | `audio-manager.js` encapsulates Web Speech API speech synthesis and Web Audio MediaRecorder with deterministic mock fallbacks for headless CI; audio recording blobs are not durably persisted in IndexedDB. | `audio-manager.js`, `tests/` |
...
|---|---|---|---|---|---|---|
| **R3-F001** | `[VERIFIED]` | `src/caption-normalizer.js`: `normalizeRawCues`, `suffixOverlap` | `tests/caption-normalizer.test.js` | WebVTT / SRT Standards | `MEASURED` (Code Inspection) | Pure string prefix/suffix matching only; no linguistic tokenization. |
| **R3-F002** | `[VERIFIED]` | `src/transcript-aggregate.js`: `canonicalSegments`, `TRANSCRIPT_TIMELINE_OVERLAP` | `tests/transcript-aggregate.test.js` | W3C WebVTT Spec | `MEASURED` (Test Suite) | Throws on any negative time delta (`startMs < previous.endMs`). |
| **R3-F003** | `[VERIFIED]` | `src/learning-contracts.js`: `learningContractDigest`, `src/caption-normalizer.js` | `tests/learning-contracts.test.js` | FNV-1a 64-bit hash standard | `MEASURED` (Deterministic Hash) | Digest sensitivity causes ID changes on any timestamp/text modification. |
| **R3-F004** | `[VERIFIED]` | `src/video-workspace.js`: `openVideoWorkspace`, `src/resolver-contracts.js` | `tests/video-workspace.test.js` | W3C Fetch / SSE Specs | `MEASURED` (Code Inspection) | Workspace consumes full transcript array in memory at mount time. |
| **R3-F005** | `[VERIFIED]` | `src/persistence.js`: `DB_NAME`, `src/ielts-persistence.js`: `IELTS_DB_NAME`, `src/v10-contracts.js`: `V10_DB_NAME` | `tests/persistence-core.test.js`, `tests/ielts-persistence.test.js`, `tests/v10-persistence.test.js` | W3C IndexedDB 3.0 & Web Locks API | `MEASURED` (Store Ledger) | 3 separate DB instances; cross-DB atomicity is software-coordinated via Web Locks. |
| **R3-F006** | `[VERIFIED]` | `src/backup-registry.js`: `BACKUP_STORE_REGISTRY`, `BACKUP_EXTERNAL_REGISTRY` | `tests/backup-registry.test.js` | W3C IndexedDB Spec | `MEASURED` (Sentinel Suite) | Exactly 59 IDB stores + 11 external surfaces registered (`FULL_BACKUP_VERSION = 6`). |
| **R3-F007** | `[VERIFIED]` | `src/evidence-policy.js`: `decideEvidence`, `normalizeAssistanceTrace` | `tests/evidence-policy.test.js` | ADR-004 (`EvidencePolicy`) | `MEASURED` (Policy Gate Suite) | Default-deny on any assistance exposure or unverified source/evaluator. |
| **R3-F008** | `[VERIFIED]` | `src/video-workspace.js`, `src/caption-normalizer.js`, `structuredClone` | `tests/video-workspace.test.js` | HTML Living Standard (Event Loop) | `POTENTIAL_MAIN_THREAD_COST` | Synchronous execution on main UI thread; unmeasured jank risk. |
| **R3-F009** | `[VERIFIED]` | `src/backup-registry.js`: line 66, `server/caption-resolver-v2.mjs` | `tests/backup-registry.test.js` | W3C Web Storage Spec | `MEASURED` (Security Audit) | API keys excluded from backups and persistent disk storage. |
| **R3-F010** | `[VERIFIED]` | `src/transcript-resolver-v2.js`, `server/caption-resolver-v2.mjs` | `tests/transcript-resolver-v2.test.js` | Loopback RFC 6761 | `MEASURED` (Network Policy) | Desktop companion restricted to `127.0.0.1`; mobile companion disabled. |
| **R3-F011** | `[VERIFIED]` | `src/transcript-resolver-v2.js`, `src/persistence.js` | `tests/transcript-resolver-v2.test.js` | GDPR / Privacy Standards | `MEASURED` (Receipt Contract) | Cloud fallback blocked without explicit versioned consent receipt. |
| **R3-F012** | `[VERIFIED]` | `src/audio-manager.js`, `tests/mocks/audio-manager.mock.js` | `tests/audio-manager.test.js` | W3C Web Audio & Speech API | `MEASURED` (Unit Tests) | Headless CI uses deterministic mock; audio recordings not durably stored in IDB. |
| **R3-F013** | `[VERIFIED]` | `src/fsrs.js`, `src/learning-contracts.js`: `LEARNING_SKILLS` | `tests/fsrs.test.js` | FSRS v6 / `ts-fsrs` (v5.4.1) | `MEASURED` (Algorithm Suite) | 5 skills supported; operates independently of IRT difficulty or BKT mastery. |
| **R3-F014** | `[VERIFIED]` | `src/weakness-profile.js`, `src/ielts-domain.js` | `tests/weakness-profile.test.js` | ADR-009 (`WeaknessProfile`) | `MEASURED` (Deterministic Reducer) | 23 error categories; explicitly yields insufficient-data state when unproven. |
| **R3-F015** | `[VERIFIED]` | `src/today-runner.js`, `src/storage-lock.js` | `tests/today-runner.test.js` | W3C Web Locks API | `MEASURED` (Lease Suite) | Single active tab lease; immutable target re-validation before launch. |
| **R3-F016** | `[INFERENCE]` | `src/caption-normalizer.js`, `src/video-workspace.js` | N/A (Architectural Deduction) | Reactive Streams / Pipeline Patterns | `UNKNOWN / REQUIRES_STAGE5_MEASUREMENT` | Pipeline decoupling reduces Main Thread contention risk subject to overhead. |
| **R3-F017** | `[INFERENCE]` | Browser Event Loop & `Worker` Web API | N/A (Concurrency Deduction) | W3C Web Workers Spec | `UNKNOWN / REQUIRES_STAGE5_MEASUREMENT` | Worker thread offloading improves UI isolation subject to clone overhead. |
| **R3-F018** | `[INFERENCE]` | `src/learning-contracts.js`, `src/caption-normalizer.js` | N/A (Identity Modeling) | Content Addressing & URN Standards | `MEASURED` (Domain Logic) | 5-dimensional separation resolves duplicate sentence text collisions. |
| **R3-F019** | `[INFERENCE]` | `src/transcript-resolver-v2.js`, `src/video-workspace.js` | N/A (Streaming Deduction) | Stream Processing Architecture | `UNKNOWN / REQUIRES_STAGE5_MEASUREMENT` | Chunked processing enables progressive learning access on long media. |
| **R3-F020** | `[INFERENCE]` | `src/migration-ledger.js`, `src/persistence.js` | N/A (Schema Evolution) | ADR-006 / ADR-008 (Forward Migration) | `MEASURED` (Migration Ledger) | Forward-only additive schema migration preserves durability across DB merge. |
| **R3-F021** | `[INFERENCE]` | `docs/research/R1_LEARNING_PRODUCT_RESEARCH_SUPPLEMENT_001.md` | N/A (Multi-Model Synthesis) | Pelánek (2017) [`SRC-S06`] | `MEASURED` (Psychometric Theory) | Decoupled architecture preserves FSRS $\neq$ BKT $\neq$ IRT $\neq$ CAT invariants. |
| **R3-F022** | `[UNKNOWN]` | `src/transcript-resolver-v2.js`, `server/caption-resolver-v2.mjs` | N/A (Unbenchmarked) | OpenAI Whisper / Whisper.cpp | `UNKNOWN / REQUIRES_STAGE5_MEASUREMENT` | Unbenchmarked latency and thermal load on low-end client hardware. |
| **R3-F023** | `[UNKNOWN]` | WTPsplit / ONNX Runtime Web | N/A (Unmeasured) | ONNX Runtime Web Standards | `UNKNOWN / REQUIRES_STAGE5_MEASUREMENT` | Unmeasured memory and initialization overhead across mobile browsers. |
| **R3-F024** | `[UNKNOWN]` | W3C StorageManager Quota API | N/A (Unmeasured) | Chromium / WebKit Quota Specs | `UNKNOWN / REQUIRES_STAGE5_MEASUREMENT` | Unmeasured audio cache eviction dynamics under storage pressure. |
...
- **Current State**: `CURRENTLY_IMPLEMENTED`.
- **Responsibility**: Clean raw subtitle cues, deduplicate identical rolling captions, merge word suffix overlaps, generate cryptographic content digests, and construct immutable transcript revisions.
...
- **Output**: Deeply frozen, canonicalized, and cryptographically digested contract instances (`learningContractDigest`).
...
flowchart TD
    subgraph CURRENT_PIPELINE [Current Substrate Pipeline]
        ...
        C3 --> C4[learningContractDigest: Hash Sentence ID]
...
- **Current Pipeline**: In `caption-normalizer.js`, if identical text occurs across distinct times, `learningContractDigest` computes different hash strings because `startMs` and `endMs` are hashed. However, if timestamps are missing or identical, hash collisions occur.
- **Target Constraint (`R3_TARGET_CONSTRAINT`)**: Separate *Semantic Content Identity* from *Source Occurrence Identity*. Identical sentences in the same source must receive identical semantic keys for lexical linking, but distinct unique occurrence IDs based on ordinal position in the source stream.
...
- **Current Pipeline**: `CAPTION_NORMALIZER_VERSION = 1` and `TRANSCRIPT_AGGREGATE_VERSION = 1` are embedded into digest hashes. Upgrading normalizer logic completely alters all downstream IDs.
- **Target Constraint (`R3_TARGET_CONSTRAINT`)**: Versioned normalizers must record `parentRevisionId` and provide translation mapping tables so historical learner progress is not orphaned upon transcript reprocessing.
...
- **Target Constraint (`R3_TARGET_CONSTRAINT`)**: Preserve strict cryptographic content addressing for transcript revisions, ensuring re-processing unchanged sources results in zero duplicate records.

#### 9. Learner Evidence Linkage Across Transcript Revisions
- **Current Pipeline**: If a transcript is edited or re-segmented, historical `Attempt` and `reviewEvents` referencing old `targetId` values no longer match new segment IDs.
- **Target Constraint (`R3_TARGET_CONSTRAINT`)**: Maintain *Learner Target Lineage* records linking historical attempt digests to canonical lexical entities and revised segment spans.
...
A central finding of this research is that semantic occurrence identity CANNOT be defined merely as `sourceId + canonicalLexicalKey`:

$$\text{CONTENT\_EQUALITY} \neq \text{OCCURRENCE\_EQUALITY}$$

Identical sentences (e.g., *"Thank you very much."*, *"As shown in the graph below..."*) frequently recur multiple times within the same media source or reading passage. Treating text content equality as occurrence equality causes state collisions, incorrect timestamp lookups, and corrupted exercise attempts.

### 5.2 Five-Dimensional Identity & Lineage Taxonomy

To guarantee robust identity across transcript revisions, re-segmentations, and multi-track learning, the architecture must distinguish five independent identity dimensions:
...
│    └── Used for: Lexical item linking, vocabulary frequency mapping     │
├─────────────────────────────────────────────────────────────────────────┤
│ 2. SOURCE OCCURRENCE IDENTITY                                           │
│    └── Unique instance of a sentence at an exact ordinal index in source │
│    └── Used for: Workspace playback, active rail cursor, sequence order │
├─────────────────────────────────────────────────────────────────────────┤
│ 3. ALIGNMENT LINEAGE                                                    │
│    └── Temporal media span (startMs, endMs, audio sample slice)         │
│    └── Used for: Media player looping, audio alignment, waveform cut    │
├─────────────────────────────────────────────────────────────────────────┤
│ 4. REVISION IDENTITY                                                    │
│    └── Immutable digest of transcript content under a normalizer schema │
│    └── Used for: Content addressing, re-import deduplication            │
├─────────────────────────────────────────────────────────────────────────┤
│ 5. LEARNER TARGET LINEAGE                                               │
│    └── Stable pointer binding past Attempts/Receipts to learning target │
│    └── Used for: FSRS review scheduling, WeaknessProfile error history  │
└─────────────────────────────────────────────────────────────────────────┘
```

| Dimension | Semantic Definition | Key Generating Inputs | Stability Across Resegmentation |
|---|---|---|---|
| **A. Semantic Content Identity** | Linguistic equivalence of the text content across sources. | Normalized lemma/sentence text string (`normalizeComparableText`). | **High**: Invariant to timestamp drifts and cue boundary changes. |
| **B. Source Occurrence Identity** | The specific $k$-th occurrence of a sentence in source $S$. | `sourceId` + `sourceOrdinalIndex` + `semanticContentKey`. | **Medium**: Stable unless whole paragraphs are added/deleted. |
| **C. Alignment Lineage** | The physical relationship to source audio/video timeline. | `sourceId` + `startMs` + `endMs` + `alignmentProvenance`. | **Low**: Changes when timestamps are corrected or aligned. |
| **D. Revision Identity** | Content-addressed cryptographic version of the transcript. | `sourceId` + `normalizerVersion` + SHA-256 digest of all segments. | **Immutable**: Any change creates a new revision node. |
| **E. Learner Target Lineage** | Durable anchor linking learner review records to target skills. | `targetType` + `canonicalTargetKey` + `lineageMappingRef`. | **Permanent**: Must survive transcript updates and schema migrations. |
...
### `R3-G004`: Overly Fragile Digest-Coupled Target Identity Lacking Semantic vs Occurrence Separation
- **CURRENT_STATE**: `CURRENT_GAP`
- **RAW_EVIDENCE**: `src/learning-contracts.js` line 50, `src/caption-normalizer.js` line 20.
- **TARGET_PROPERTY**: 5-dimensional identity model separating semantic content, source occurrence, alignment lineage, revision digest, and learner target linkage.
- **IMPACT**: Duplicate sentences collide; transcript re-segmentation or 1ms timestamp shifts orphan historical FSRS review progress.
- **CONFIDENCE**: `HIGH`
- **R1_RELATION**: Durable long-term retention tracking across curriculum updates.
...
- `src/transcript-aggregate.js` — Canonical segment construction, timeline overlap validation, and revision hashing.
