...
**Remediation Candidate: STAGE3-R3-PIPELINE-ARCHITECTURE-RESEARCH-REM-002**

- **Document Identifier**: `R3_PIPELINE_ARCHITECTURE_RESEARCH.md`
- **Transaction Identity**: `STAGE3-R3-PIPELINE-ARCHITECTURE-RESEARCH-REM-002`
- **Historical Subjects / Superseded Candidates**: PR #160 (`c3793931c29eaeb85ecad3b38622f19c1da2cd53`), PR #161 (`56616336f2e6dbaee5f16b2e6eb15a395371ffaf`)
...
| **R3-F003** | `[VERIFIED]` | Identity / Digest | Sentence and segment IDs are derived from deterministic 64-bit FNV-1a non-cryptographic digests of exact canonicalized text and timestamps (`learningContractDigest`); any algorithmic re-segmentation or timestamp shift changes the resulting digest and invalidates downstream target links unless explicit lineage mapping is maintained. | `learningContractDigest` (`src/learning-contracts.js`, `src/caption-normalizer.js`) |
...
| **R3-F003** | `[VERIFIED]` | `src/learning-contracts.js`: `learningContractDigest`, `src/caption-normalizer.js` | `tests/learning-contracts.test.mjs` | FNV-1a 64-bit non-cryptographic digest algorithm | `DETERMINISTIC_COMPUTATION_VERIFIED` | Deterministic identity digest ($\neq$ cryptographic integrity digest); changes on any timestamp/text modification. |
...
| **R3-F018** | `[INFERENCE]` | `src/learning-contracts.js`, `src/caption-normalizer.js` | N/A (Identity Modeling) | Content Addressing & URN Standards | `STATIC_CODE_VERIFIED` | 5-dimensional separation resolves duplicate sentence text collisions. |
...
- **Current Pipeline**: In `caption-normalizer.js`, if identical text occurs across distinct times, `learningContractDigest` computes different deterministic 64-bit FNV-1a digest strings because `startMs` and `endMs` are included in the digested payload. However, if timestamps are missing or identical, digest collisions occur across occurrences.
...
- **Current Pipeline**: `CAPTION_NORMALIZER_VERSION = 1` and `TRANSCRIPT_AGGREGATE_VERSION = 1` are embedded into digest inputs. Upgrading normalizer logic completely alters all downstream FNV-1a digest IDs.
...
| **D. Revision Identity** | Content-addressed version of the transcript (current: deterministic 64-bit FNV-1a digest; future proposal: cryptographic digest). | `sourceId` + `contentDigest` (`fnv1a64:...` in `src/transcript-aggregate.js`; target proposal: cryptographic hash). | **Immutable**: Any change creates a new revision node. |
