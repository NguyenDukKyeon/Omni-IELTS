...
| Field | Value |
|---|---|
| Transaction | STAGE4-W4-DESIGN-SYSTEM-001 |
| Human gate | G4 ACTIVATED FOR W4 CANDIDATE EXECUTION |
...
| Artifact class | Design System & Token Specification; non-runtime implementation |
| Closed write allowlist | `docs/stage4/STAGE4_DESIGN_SYSTEM.md` only |
| Epistemic status | W4 candidate pending independent audit; CI is not acceptance |
...
[ LAYER 1: PRIMITIVE TOKENS ]   (e.g., slate-900, indigo-600, amber-500, emerald-600)
             │
             ▼
[ LAYER 2: SEMANTIC TOKENS ]    (e.g., color-bg-surface, color-text-primary, color-assistance-scaffold)
             │
             ▼
...
| Semantic Token | Light Mode Value | Dark Mode Value | Role / Usage |
|---|---|---|---|
| `--color-exam-bg-canvas` | `#f1f5f9` | `#0f172a` | Strict test canvas background |
| `--color-exam-bg-surface` | `#ffffff` | `#1e293b` | Test content panels, split passage/question |
| `--color-exam-header-bg` | `#1e293b` | `#020617` | Official exam top bar (high contrast dark) |
| `--color-exam-header-text` | `#f8fafc` | `#f8fafc` | Exam title, candidate ID, clock |
| `--color-exam-border-divider` | `#cbd5e1` | `#334155` | Split pane vertical divider, section edges |
| `--color-exam-timer-normal` | `#f8fafc` | `#f8fafc` | Standard countdown display |
| `--color-exam-timer-warning` | `#f59e0b` | `#f59e0b` | 10-minute warning state |
| `--color-exam-timer-critical`| `#f43f5e` | `#f43f5e` | 5-minute final urgency state |
| `--color-exam-palette-unanswered` | `#ffffff` (slate-300 border) | `#1e293b` (slate-600 border) | Unanswered question item |
| `--color-exam-palette-answered`   | `#334155` (text white) | `#64748b` (text white) | Answered question item |
| `--color-exam-palette-flagged`    | `#fef3c7` (amber-700 border) | `#78350f` (amber-400 border) | Flagged for review item |
| `--color-exam-palette-current`    | `#4f46e5` (ring-2) | `#818cf8` (ring-2) | Active active question item |

### 3.4 Assistance & Evidence Semantic Tokens

| Token | Meaning / Classification | Foreground Color | Background Color | EvidencePolicy Consequence |
|---|---|---|---|---|
| `--color-assist-ua` | **UNASSISTED** (Independent) | `--primitive-slate-700` | `--primitive-slate-100` | **Eligible** for positive FSRS memory update & skill evidence |
| `--color-assist-la` | **LIGHT_ASSISTANCE** (Hint/Gloss) | `--primitive-sky-700` | `--primitive-sky-50` | Qualified practice record; **no unassisted credit** |
| `--color-assist-sc` | **SCAFFOLDED** (Frame/IPA/Model) | `--primitive-amber-700` | `--primitive-amber-50` | Scaffolding record; **no unassisted credit** |
| `--color-assist-ar` | **ANSWER_REVEALED / REVIEW** | `--primitive-purple-700` | `--primitive-purple-50` | Post-attempt review; **0% independent recall evidence** |
| `--color-evidence-high` | High Confidence / Validated | `--primitive-emerald-700` | `--primitive-emerald-50` | Robust grounded evidence receipt |
| `--color-evidence-med` | Moderate / Single Occurrence | `--primitive-amber-700` | `--primitive-amber-50` | Qualified provisional estimate |
| `--color-evidence-low` | Sparse / Self-Reported / Stale | `--primitive-slate-600` | `--primitive-slate-100` | Low-certainty indicator |
...
- `--target-touch-min`: `48px` $\times$ `48px` (Mandatory mobile touch envelope)
- `--target-desktop-min`: `40px` $\times$ `40px` (Desktop interactive button/action height)
- `--target-compact-min`: `32px` $\times$ `32px` (Dense table action, audio chip, palette item)
- `--input-height-default`: `44px` (Form fields, dropdowns, search bars)
...
:root {
  ...
  --state-focus-ring-exam:  0 0 0 2px #ffffff, 0 0 0 4px #0f172a;
...
```
[ UA: Unassisted ]  -> Neutral Slate badge (Indicates independent performance)
[ LA: Light Assist ]-> Sky Blue outlined badge (Hint, Glossary, or Pronunciation preview used)
[ SC: Scaffolded ]  -> Amber badge (Sentence frame, IPA thought-group guide, or outline active)
[ AR: Revealed ]    -> Purple badge (Answer revealed, transcript checked, or post-attempt review)
```
...
+─────────────────────────────────────────────────────────────+
| Exposure: UNASSISTED (UA)                                   |
| Evaluator: Local Rule Engine v1.4                           |
| Confidence: HIGH (±0.04)                                    |
| Memory Consequence: FSRS S=4.2d, R=91% (Scheduled)          |
| Skill Construct: LEXICAL_ACTIVE_PRODUCTION (Formative only) |
+─────────────────────────────────────────────────────────────+
| < Default Deny: Coaching & revealed answers emit no credit >|
+─────────────────────────────────────────────────────────────+
```
...
- Accessible Item Tokens:
  ...
  - `Current`: Highlighted with 2px indigo-500 outer ring.
...
- In strict exam mode, correct answer keys, explanations, and model essays are completely stripped from client response payloads and accessibility trees.
...
- Visual token: Active cue highlighted with `indigo-50` background and 3px `indigo-600` left indicator bar.
...
- Four rating buttons with clear, honest cognitive descriptors:
  - `Again` (< 10m): Complete recall failure / incorrect response.
  - `Hard` (1.2d): Recalled with significant effort / partial delay.
  - `Good` (3.5d): Successful recall with normal hesitation.
  - `Easy` (7.0d): Instant, effortless recall.
- Token rule: Rating buttons display projected next interval in subscript to eliminate scheduling opacity.
...
- Focus Ring: 2px solid `indigo-600` with 2px offset.
- Error State: 1.5px solid `rose-500` border, `rose-50` background tint, inline error message linked via `aria-describedby`.
- Success State: 1px solid `emerald-500` border, inline checkmark icon.

### 15.2 Buttons & Action Triggers
- `ButtonPrimary`: Solid indigo-600 background, white text, 8px radius, semibold font.
- `ButtonSecondary`: White background, 1px slate-300 border, slate-800 text.
- `ButtonDestructive`: Rose-600 background, white text, requires 2-step confirmation or 5-second undo toast.
- `ButtonGhost`: Transparent background, slate-700 text, hover slate-100 tint.
...
| **Core-Only Degraded Mode** | Persistent top notification banner (`#fffbeb`) | `[ View Service Status ]` | Local drills functional; cloud AI queued |
...
| Foreground Token | Background Token | Measured Contrast Ratio | WCAG Compliance Level |
|---|---|---|---|
| `--color-learn-text-primary` (`#0f172a`) | `--color-learn-bg-surface` (`#ffffff`) | **16.1 : 1** | **Passes AAA** (Threshold: 7.0:1) |
| `--color-learn-text-secondary` (`#475569`) | `--color-learn-bg-surface` (`#ffffff`) | **6.8 : 1** | **Passes AA** (Threshold: 4.5:1) |
| `--color-learn-brand-primary` (`#4f46e5`) | `--color-learn-bg-surface` (`#ffffff`) | **5.9 : 1** | **Passes AA** (Threshold: 4.5:1) |
| `--color-exam-header-text` (`#f8fafc`) | `--color-exam-header-bg` (`#1e293b`) | **13.4 : 1** | **Passes AAA** (Threshold: 7.0:1) |
| `--color-assist-ua` (`#334155`) | `--color-assist-ua-bg` (`#f1f5f9`) | **7.4 : 1** | **Passes AAA** (Threshold: 7.0:1) |
| `--color-assist-sc` (`#b45309`) | `--color-assist-sc-bg` (`#fffbeb`) | **5.2 : 1** | **Passes AA** (Threshold: 4.5:1) |
...
- [x] EvidencePolicy default-deny visual feedback implemented across all practice surfaces.
- [x] Mobile touch targets verified at $\ge 48\times 48\text{px}$.
- [x] Color contrast ratios verified $\ge 4.5:1$ (AA) and $\ge 7:1$ (AAA).
...
| **S4-OMIT-003** | Strict dictation ARIA masking | `AntiCheatingMask` stripping answers from DOM/ARIA before submit | **PRESERVED** |
...
Candidate terminal state: `W4_DESIGN_SYSTEM_CANDIDATE_COMPLETE_PENDING_INDEPENDENT_AUDIT`.
