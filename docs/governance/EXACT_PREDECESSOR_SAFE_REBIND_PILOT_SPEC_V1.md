...
**Transaction ID**: `AGENT-HARNESS-EXACT-PREDECESSOR-SAFE-REBIND-PILOT-SPEC-002-REMEDIATION`  
**Historical Rejected Candidate**: PR #106 (`63d44d030077e0cf4d68fa38558be69bf40cf4c1` / Review `PRR_kwDOTmjPCs8AAAABJs64LA`)  
...
### 5.2 Footprint Categories & Extraction Ledger

Pilot V0 supports manifest format **`BOUNDED_EXECUTION_CAPSULE_PROTOCOL_V1`** and binds to exact canonical section headings:

| Footprint Category | Definition & Scope | Canonical Manifest Extraction Anchor | Identity Requirement | Failure Classification |
|---|---|---|---|---|
| **1. AUTHORITY_FOOTPRINT** | Authorization manifest, accepted blob SHA, acceptance anchor commit, controlling Protocol & ADRs, canonical status facts. | Parsed from manifest header, `docs/authorizations/`, `docs/DECISIONS.md`, and `docs/IMPLEMENTATION_STATUS.md`. | Blob SHA immutable from acceptance anchor $K$ through $B$. | Modified $\to$ `REBIND_UNSAFE`<br/>Ambiguous $\to$ `REBIND_UNKNOWN` |
| **2. WRITE_FOOTPRINT** | Exact file paths authorized for modification in candidate transaction. | `### 5.1 Source Allowlist (SOURCE_ALLOWLIST)`, `### 5.2 Test Allowlist (TEST_ALLOWLIST)`, and `### 5.3 Governance & Status Allowlist (GOVERNANCE_ALLOWLIST)` tables (`File Path` column). | Zero tree mutations (`add`, `modify`, `delete`, `rename`) across $(A \dots B]$. | Modified $\to$ `REBIND_UNSAFE`<br/>Rename Ambiguity $\to$ `REBIND_UNKNOWN` |
| **3. REQUIRED_READ_FOOTPRINT** | Canonical documents, domain models, and specs required to be read by the transaction. | `## 1.2 Controlling Authorities Fresh-Read Ledger` and `### 5.4 Read-Only Reference Context (READ_ONLY_CONTEXT)` (Explicit path tokens only). | Exact content blob SHA equality across $(A \dots B]$. | Modified $\to$ `REBIND_UNSAFE`<br/>Unresolved $\to$ `REBIND_UNKNOWN` |
| **4. TEST_FOOTPRINT** | Target test suites, unit tests, browser harnesses, test helpers, and test fixtures. | `### 5.2 Test Allowlist (TEST_ALLOWLIST)` + `## 9. Local Verification and Regression Gates` + AST imports from test files. | Zero diff across test files, helpers, fixtures, and harnesses in $(A \dots B]$. | Modified $\to$ `REBIND_UNSAFE`<br/>Incomplete Graph $\to$ `REBIND_UNKNOWN` |
...
| **7. DATA_SCHEMA_FOOTPRINT** | Database version constants, migration files, migration ledger, backup registry, restore schemas. | `## 4. Persistence Model and Store Manifest` (Includes `src/*persistence*.js`, `src/backup-registry.js`; explicitly `EMPTY` for non-persistence tasks). | Exact equality of database versions, migration tables, and backup registry contracts in $(A \dots B]$. | Modified $\to$ `REBIND_UNSAFE` |
...
### 7.2 Write Footprint & Tree Mutations
The Write Footprint tracks Git tree mutations across $(A \dots B]$:
- `add`: Path added in $(A \dots B]$ matching Write Footprint $\to$ `REBIND_UNSAFE` (`WRITE_FOOTPRINT_CHANGED`).
- `modify`: Path modified in $(A \dots B]$ matching Write Footprint $\to$ `REBIND_UNSAFE` (`WRITE_FOOTPRINT_CHANGED`).
- `delete`: Path deleted in $(A \dots B]$ matching Write Footprint $\to$ `REBIND_UNSAFE` (`WRITE_FOOTPRINT_CHANGED`).
- `rename`: Path renamed in $(A \dots B]$ involving Write Footprint $\to$ `REBIND_UNSAFE` (`WRITE_FOOTPRINT_CHANGED`).
...
The `REQUIRED_READ_FOOTPRINT` is mechanically extracted from explicit markdown file paths found under:
- `## 1.2 Controlling Authorities Fresh-Read Ledger`
- `### 5.4 Read-Only Reference Context (READ_ONLY_CONTEXT)`

**Extraction Algorithm**:
1. Scan table cells and bulleted lists under the designated headings.
2. Extract all code-quoted or linked file paths matching `*.md` or repository file patterns (e.g. `docs/MASTER_ROADMAP.md`, `AGENTS.md`).
3. Normalize to POSIX relative paths without `./`.
4. Deduplicate and sort lexicographically.
5. Verify that each file exists in $\text{TREE}(A)$ and $\text{TREE}(K)$.
6. Zero open-ended heuristic discovery; zero LLM relevance expansion.
7. If any required read path is malformed or unresolvable $\to$ `REBIND_UNKNOWN` (`FOOTPRINT_INCOMPLETE`).

### 7.4 RED Contract Viability Preflight & Safe Gate
...
| Predicate not evaluated / skipped | `NOT_EVALUATED` | **`REBIND_UNKNOWN`** (Treated as `UNKNOWN` for mandatory predicates) | `RED_PREDICATE_UNKNOWN` |
...
## 9. Four-State Classification Semantics
  ...
  "properties": {
    ...
    "intervening_path_changes": {
      ...
      "items": {
        ...
        "properties": {
          "path": { "type": "string" },
          "status": { "type": "string", "enum": ["A", "M", "D", "R", "C"] },
...
Every property in the JSON schema is normatively specified in this ledger ($\text{JSON\_SCHEMA\_PROPERTIES} = \text{FIELD\_LEDGER\_PROPERTIES}$):
...
| `intervening_path_changes` | Normalized endpoint tree delta $A \to B$ | Array of Objects | Required | Lexicographically sorted by `path` | Status in `[A, M, D, R, C]` | Empty if $A == B$; uncomputable $\to$ `REBIND_UNKNOWN` |
...
| `reason_codes` | List of applicable reason codes | Array of Enums | Required | Deduplicated, sorted ASCII ascending | Enum values from §12 | Empty for `REBIND_SAFE` |
...
Because Git histories on `main` contain merge commits, `intervening_commits` is defined over the directed acyclic graph (DAG):
...
2. **Deterministic Tie-Break Rule**: Among multiple currently eligible commits having zero unsatisfied parent dependencies in $S$, the tie-break selects the commit with the lexicographically smallest lowercase 40-character SHA string.
...
### 11.2 Normalized Endpoint Tree Path-Delta Semantics
`intervening_path_changes` records the normalized endpoint tree delta from $\text{TREE}(A)$ to $\text{TREE}(B)$:
1. **Delta Method**: Evaluates exact endpoint differences (equivalent to `git diff-tree -r -z A B`).
2. **Rename Recognition**: Rename status (`R`) is assigned **only** when exact content identity is proven (`old_blob_sha == new_blob_sha`). Heuristic renames below 100% similarity are represented as separate Delete (`D`) and Add (`A`) entries (or `FOOTPRINT_INCOMPLETE` if path identity cannot be established).
...
The normative reason code taxonomy and the JSON Schema enum are strictly identical:
...
| **`WRITE_FOOTPRINT_CHANGED`** | UNSAFE | One or more files in Write Footprint were modified in $(A \dots B]$. |
...
### 15.2 Single Deterministic Replay Classification
- **Expected Pilot Result**: **`REBIND_UNSAFE`**
- **Expected Reason Codes**: `["WRITE_FOOTPRINT_CHANGED"]`
- **Derivation & Benchmarking Value**: In the range $(A \dots B]$, PR #91 modified `src/ielts-hub-v2.js`, which is an explicit member of W0's Write Footprint (`SOURCE_ALLOWLIST`). Under Pilot V0's conservative rules, a known modification to a write allowlist file yields `REBIND_UNSAFE`. This benchmark measures **Conservatism and False-Unsafe Cost**, proving that the machine checker strictly rejects write-footprint modifications without heuristic relaxation.
...
To verify that the checker is not a trivial rejector (returning UNKNOWN/UNSAFE for everything), the test suite contains at least one positive SAFE fixture:
...
- [x] Conservative V0 assumption footprint is fully decomposed into 9 distinct categories with exact canonical manifest extraction bindings.
- [x] Path disjointness is explicitly rejected as a sufficient proof of safety.
- [x] RED contract viability preflight and complete safe gate truth table are mandatory.
...
- [x] Field Specifications Ledger has exact property-set equality with JSON Schema ($\text{SCHEMA\_PROPERTIES} = \text{FIELD\_LEDGER\_PROPERTIES}$).
- [x] Standardized reason code taxonomy and JSON Schema enum have exact set equality.
- [x] Deterministic topological merge-DAG ordering contract is defined.
- [x] Intervening endpoint tree path-delta semantics are exact.
- [x] Acceptance anchor commit $K$ resolution contract is mathematically verifiable.
- [x] W0 historical replay case is specified with exact historical identities ($A=\text{a755ae4}$, $K=\text{ee7d1b7}$, $B=\text{4130ef9}$) and single deterministic result `REBIND_UNSAFE` (`WRITE_FOOTPRINT_CHANGED`).
