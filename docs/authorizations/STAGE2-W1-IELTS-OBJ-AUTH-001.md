...
Manifest Identity: **STAGE2-W1-IELTS-OBJ-AUTH-001**  
...
This authorization manifest is governed strictly by the repository authority hierarchy established in `docs/MASTER_ROADMAP.md`:
...
\downarrow \\
\textbf{Level 3: This Bounded Wave Authorization } (\textbf{STAGE2-W1-IELTS-OBJ-AUTH-001}) \\
\downarrow \\
\text{Level 4: Future W1 Bounded Execution } (\text{Commit A RED } \to \text{ Commit B GREEN}) \\
\downarrow \\
...
### 3.3 Full 26 Task-Family Ledger and Coverage Mapping

The following ledger establishes the exact machine-auditable mapping of all 11 official Listening and 15 official Reading task families (Academic and General Training unified) under the canonical QAR substrate:

#### 3.3.1 Listening Task Families (11 / 11 Official Families)

| # | Official Task Family | Canonical QAR Kind | Execution Primitive | Scoring Primitive | Test Evidence | Reused / New Substrate |
|---|---|---|---|---|---|---|
...
#### 3.3.2 Reading Task Families (15 / 15 Official Families — Academic & GT Unified)
...
| R10 | **Summary Completion (Text)** | `reading-summary-completion` (v1) | `ObjectiveTextResponse` (OTR) | `OBJECTIVE_TEXT_RESPONSE_SCORER` (v1) | `tests/qar-objective-text-response.test.mjs`, `tests/wave4-ielts-reading-objective-text.test.mjs` | REUSED (Stage 1 / Wave 4) |
| R11 | **Summary Completion (Box)** | `reading-summary-completion-box` (v1) | `ObjectiveMatchingResponse` (Option Pool) | `OBJECTIVE_MATCHING_RESPONSE_SCORER` (v1) | `tests/qar-objective-matching-response.test.mjs` | **NEW in W1 (GAP-03)** |
| R12 | **Note Completion** | `reading-note-completion` (v1) | `ObjectiveTextResponse` (OTR) | `OBJECTIVE_TEXT_RESPONSE_SCORER` (v1) | `tests/qar-objective-text-response.test.mjs`, `tests/wave4-ielts-reading-objective-text.test.mjs` | REUSED (Stage 1 / Wave 4) |
| R13 | **Table Completion** | `reading-table-completion` (v1) | `ObjectiveTextResponse` (OTR) | `OBJECTIVE_TEXT_RESPONSE_SCORER` (v1) | `tests/qar-objective-text-response.test.mjs`, `tests/wave4-ielts-reading-objective-text.test.mjs` | REUSED (Stage 1 / Wave 4) |
| R14 | **Flow-Chart Completion** | `reading-flow-chart-completion` (v1) | `ObjectiveTextResponse` (OTR) | `OBJECTIVE_TEXT_RESPONSE_SCORER` (v1) | `tests/qar-objective-text-response.test.mjs`, `tests/wave4-ielts-reading-objective-text.test.mjs` | REUSED (Stage 1 / Wave 4) |
| R15 | **Diagram Label Completion** | `reading-diagram-label-completion` (v1) | `ObjectiveTextResponse` + Spatial | `OBJECTIVE_TEXT_RESPONSE_SCORER` + Spatial | `tests/qar-objective-text-response.test.mjs`, `tests/wave4-ielts-reading-spatial.test.mjs` | REUSED (Stage 1 / Wave 4) |
| R16 | **Short-Answer Questions** | `reading-short-answer` (v1) | `ObjectiveTextResponse` (OTR) | `OBJECTIVE_TEXT_RESPONSE_SCORER` (v1) | `tests/qar-objective-text-response.test.mjs`, `tests/wave4-ielts-reading-objective-text.test.mjs` | REUSED (Stage 1 / Wave 4) |

*Note on official taxonomy:* Summary Completion has two official variants: free text extracted from the passage (R10) and option-pool matching from a given box (R11). Together with the other 14 families, they comprise the complete 15 official IELTS Reading task families.
...
1. **Selection Cardinality:** A question defines expected selection count $N$ (where $N \ge 2$) from a total of $M$ options (where $M > N$, typically 5 choices for pick-2, or 7 choices for pick-3).
2. **Declaration Invariants:** Exactly $N$ options in the sealed definition have `correct: true`.
3. **Response Normalization:**
   - Learner response must be an array of unique option IDs: `{ optionIds: string[] }`.
   - Duplicate option IDs in submission are strictly rejected with `QUESTION_ACTIVITY_RESPONSE_INVALID`.
   - Accessor-backed or hostile objects fail closed without getter invocation.
...
4. **Deterministic Scoring & Partial Credit:**
   ...
   - Let $K$ be the number of submitted option IDs that match `correct: true` options.
   - Let $U$ be the number of submitted option IDs that match `correct: false` options.
   - $\text{numerator} = K$.
   - $\text{denominator} = N$.
   - **Score Disposition:**
   ...
   - **Under-Selection / Over-Selection:** Submissions with $\text{count} \ne N$ fail validation with `QUESTION_ACTIVITY_RESPONSE_INVALID` or score with exact denominator $N$ and zero extra credit for over-selection.
5. **Receipt / Metadata Binding:** Emits canonical `questionResult` metadata containing `{ normalizedResponse, disposition, numerator, denominator, scorer, keyDigest, reviewRequired: false, affectsSchedule: false }`.
...
6. **Rejection of Invalid Submissions:** Duplicate options submitted under `SINGLE_USE`, unknown option IDs, unknown slot IDs, or missing slots fail closed.
...
|---|---|---|
| `src/question-activity-contracts.js` | Extend QAR registry, validation, execution, and coverage report for `multiple-choice-multiple` (GAP-02) and `summary-completion-box` (GAP-03); export constants and entries for all 26 task families. | EXTEND |
...
| File Path | Ownership Reason | Change Class |
|---|---|---|
| `tests/qar-00-question-activity-contracts.test.mjs` | Unit and integration tests for multi-select MCQ, QAR registry extensions, 26 task family coverage report, and sealed key privacy. | EXTEND |
| `tests/qar-objective-matching-response.test.mjs` | Unit tests for `summary-completion-box` option-pool matching, slot bindings, reuse policies, and tamper-resistance. | EXTEND |
| `tests/qar-objective-text-response.test.mjs` | Unit tests for objective text normalization, word count enforcement, and completion item contracts. | EXTEND |
...
| `tests/qar-00-question-activity-contracts.test.mjs` | Assert coverage report `getQuestionCoverageReport()` includes all 26 official task families without gaps in registered kinds. | Missing kind entries or incomplete coverage report count |
...
4. Invalid selections (unknown option IDs, duplicate choices, single-use violations) fail closed safely.
5. All 26 official task-family mappings validate, execute, score, and emit canonical receipts under QAR contracts.
...
| `Authorization Manifest` | `docs/authorizations/STAGE2-W1-IELTS-OBJ-AUTH-001.md` |
| `Authorization Manifest Identity` | `STAGE2-W1-IELTS-OBJ-AUTH-001` |
...
6. Exactly ONE changed path: `docs/authorizations/STAGE2-W1-IELTS-OBJ-AUTH-001.md`;
