...
const gates=['test:phase2','check','build','test:backup','test:restore'];const started=performance.now();
for(const [index,script] of gates.entries()){
  console.log(`\n[PHASE2 VERIFY ${index+1}/${gates.length}] ${script}`);const result=spawnSync(process.execPath,[npmCli,'run',script],{cwd:process.cwd(),env:process.env,stdio:'inherit',windowsHide:true});
  if(result.error||result.status!==0){console.error(`[GATE_FAILURE] ${script} failed.`);process.exit(result.status||1);}
}
console.log(`\nPHASE2 IMPLEMENTATION VERIFICATION PASSED: ${gates.length}/${gates.length} gates in ${((performance.now()-started)/1000).toFixed(1)}s.`);
