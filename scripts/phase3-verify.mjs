...
const gates=['test:phase3'];const started=performance.now();
for(const [index,script] of gates.entries()){
  console.log(`\n[PHASE3 VERIFY ${index+1}/${gates.length}] ${script}`);
...
console.log(`\nPHASE3 VERIFICATION PASSED: ${gates.length}/${gates.length} gates in ${((performance.now()-started)/1000).toFixed(1)}s.`);
