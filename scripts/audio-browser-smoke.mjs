...
try{
  ...
  await page.click('#wordDetailDialog [data-close-dialog]');await page.click('.side-nav [data-route="today"]');await page.click('#openMorePractice');await page.click('[data-practice="listening"]');await page.waitForSelector('#studyOverlay.open');const visible=(await page.locator('#exerciseHost').textContent())||'';assert.doesNotMatch(visible,/efficient/i);await page.click('#inlineSlowAudio');await page.waitForFunction(()=>window.__audioLog.at(-1)?.rate===0.7);
