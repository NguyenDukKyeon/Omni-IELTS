...
async function main(){
  await withBrowserHarness({name:'Phase 5 browser smoke',profilePrefix:'vocab-phase5-smoke-',ports:[APP_PORT,DEBUG_PORT]},async({browser,profileDir,spawnTracked})=>{
    ...
    try{
      ...
      const waitForValue=async(expression,label)=>{for(let attempt=0;attempt<180;attempt+=1){const value=await evaluate(expression);if(value)return value;await delay(100);}throw new Error(`Timed out waiting for ${label}.`);};

      await cdp.send('Emulation.setDeviceMetricsOverride',{width:1280,height:900,deviceScaleFactor:1,mobile:false});
      await waitFor('globalThis.innerWidth===1280','desktop viewport');
      await waitFor('window.__VOCAB_V10_READY__&&window.VocabMasterIeltsHub&&window.VocabMasterTranscriptResolver','Phase 5 runtime');
      await evaluate('window.VocabMasterIeltsHub.open("videos")');
      const desktop=await waitForValue(`(()=>{const device=document.querySelector('#v10IeltsHubDialog [data-phase5-device="desktop"]'),root=device?.closest('[data-phase5-fallback]');if(!root?.querySelector('[data-phase5-import-submit]'))return null;return{device:device.dataset.phase5Device,local:Boolean(root.querySelector('[data-phase5-local]')),localChecked:root.querySelector('[data-phase5-local]')?.checked===true,cloudDisabled:root.querySelector('[data-phase5-cloud]')?.disabled===true,cloudChecked:root.querySelector('[data-phase5-cloud]')?.checked===true,hasDisclosure:/Dữ liệu:/.test(root.textContent||''),hasImport:true};})()`,'desktop fallback UI');
      ...
      await evaluate(`(()=>{const root=document.querySelector('#v10IeltsHubDialog [data-phase5-device="desktop"]')?.closest('[data-phase5-fallback]');root.open=true;root.querySelector('[data-phase5-import-text]').value='00:00:00,000 --> 00:00:02,000\\nBrowser imported sentence.\\n\\n00:00:02,000 --> 00:00:04,000\\nDurable after reload.';root.querySelector('[data-phase5-import-submit]').click();})()`);
      const importStatus=await waitForValue(`(()=>{const root=document.querySelector('#v10IeltsHubDialog [data-phase5-device="desktop"]')?.closest('[data-phase5-fallback]'),status=root?.querySelector('[data-phase5-status]')?.textContent?.trim();return status||null;})()`,'import result');
      assert.match(importStatus,/Đã import/,`Import failed in the production UI: ${importStatus}`);
      ...
      await cdp.send('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true});await waitFor('globalThis.innerWidth===390','mobile viewport');await evaluate('window.VocabMasterIeltsHub.open("videos")');
      let mobile;
      try{mobile=await waitForValue(`(()=>{const device=document.querySelector('#v10IeltsHubDialog [data-phase5-device="mobile"]'),root=device?.closest('[data-phase5-fallback]');if(!root?.querySelector('[data-phase5-import-submit]'))return null;return{width:document.documentElement.clientWidth,device:device.dataset.phase5Device,local:Boolean(root.querySelector('[data-phase5-local]')),hasImport:true,copy:root.textContent?.includes('không chạy yt-dlp/Whisper cục bộ')===true};})()`,'mobile fallback UI');}
      catch(error){const snapshot=await evaluate(`(()=>({innerWidth:globalThis.innerWidth,clientWidth:document.documentElement.clientWidth,devices:[...document.querySelectorAll('#v10IeltsHubDialog [data-phase5-device]')].map(node=>node.dataset.phase5Device),fallbacks:document.querySelectorAll('#v10IeltsHubDialog [data-phase5-fallback]').length,panel:document.querySelector('#v10IeltsHubPanel')?.textContent?.slice(0,500)||''}))()`);error.message+=`\nMobile DOM snapshot: ${JSON.stringify(snapshot)}`;throw error;}
