...
async function submitAnswer(answer){
  ...
  const commit=async({finalRating,finalStatus,addCorrective=false,acceptedVariant=''})=>{
    ...
    const acceptedSkill=resolvedSkill(step,current);const acceptedForSkill=Array.isArray(current.acceptedBySkill?.[acceptedSkill])?current.acceptedBySkill[acceptedSkill]:[];
    if(acceptedVariant&&!acceptedForSkill.includes(acceptedVariant)){
      current=replaceCard({...current,acceptedBySkill:{...(current.acceptedBySkill||{}),[acceptedSkill]:[...acceptedForSkill,acceptedVariant]}});
...
async function confirmImport(){if(!state.importResult)return;const{cards,updates}=state.importResult;if(!cards.length&&!updates.length)return;await persistImportBatch({cards,updates},'import-atomic');state.cards=[...cards,...state.cards.map(card=>updates.find(item=>item.id===card.id)||card)];renderAll();showToast(`Đã thêm ${cards.length} và cập nhật ${updates.length} mục trong một giao dịch.`);closeDialog($('#importDialog'));}
