...
export async function openVideoWorkspace(row,{startIndex=0,mode='normal'}={}){
  if(row?.aligned===false||row?.metadata?.aligned===false||row?.segments?.some(segment=>segment.aligned===false))throw Object.assign(new Error('Transcript chưa có timing thật nên không thể dùng cho hoạt động phụ thuộc thời gian.'),{code:'TRANSCRIPT_UNALIGNED'});if(!row?.videoId||!row?.segments?.length)throw new Error('Video chưa có transcript để học.');ensureStyles();resetWorkspaceLayout();
