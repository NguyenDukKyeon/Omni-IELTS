...
function upgradeLegacyV2Envelope(input){
  ...
  const legacyIeltsStores=ownerEntries('ielts').filter(row=>!['ieltsTestBlueprints','ieltsTestRuns'].includes(row.store)).map(row=>row.store);
  for(const owner of ['core','ielts','v10']){
  ...
  domains.ielts.stores[IELTS_STORE_NAMES.testBlueprints||'ieltsTestBlueprints']=[];
  domains.ielts.stores[IELTS_STORE_NAMES.testRuns||'ieltsTestRuns']=[];
  domains.ielts.database=IELTS_DB_NAME;
  domains.ielts.databaseVersion=IELTS_DB_VERSION;
  return{
...
function upgradeRegistryV2Envelope(input){
  ...
  const legacyIeltsStores=ownerEntries('ielts').filter(row=>![IELTS_STORE_NAMES.objectiveInventory,IELTS_STORE_NAMES.learnerArtifacts,'ieltsTestBlueprints','ieltsTestRuns'].includes(row.store)).map(row=>row.store);
  const legacyV10Stores=ownerEntries('v10').filter(row=>row.store!==V10_STORES.privateSources).map(row=>row.store);
  for(const owner of ['core','ielts','v10']){
...
function upgradeRegistryV3Envelope(input){
  ...
  const expectedRows=BACKUP_STORE_REGISTRY.filter(row=>!(row.owner==='ielts'&&[IELTS_STORE_NAMES.learnerArtifacts,'ieltsTestBlueprints','ieltsTestRuns'].includes(row.store))&&!(row.owner==='v10'&&row.store===V10_STORES.privateSources)).map(row=>row.owner==='ielts'?{...row,databaseVersion:2}:row.owner==='v10'?{...row,databaseVersion:7}:row);
  ...
  domains.ielts.stores[IELTS_STORE_NAMES.learnerArtifacts]=[];domains.ielts.stores[IELTS_STORE_NAMES.testBlueprints||'ieltsTestBlueprints']=[];domains.ielts.stores[IELTS_STORE_NAMES.testRuns||'ieltsTestRuns']=[];domains.ielts.database=IELTS_DB_NAME;domains.ielts.databaseVersion=IELTS_DB_VERSION;domains.v10.stores[V10_STORES.privateSources]=[];
...
function upgradeRegistryV4Envelope(input){
  ...
  const expectedRows=BACKUP_STORE_REGISTRY.filter(row=>!(row.owner==='v10'&&row.store===V10_STORES.privateSources)&&!(row.owner==='ielts'&&['ieltsTestBlueprints','ieltsTestRuns'].includes(row.store))&&(hasFrozen||!(row.owner==='ielts'&&row.store===IELTS_STORE_NAMES.frozenAssessments))).map(row=>row.owner==='v10'?{...row,databaseVersion:7}:row.owner==='ielts'?{...row,databaseVersion:3}:row);
  ...
  domains.ielts.stores[IELTS_STORE_NAMES.frozenAssessments]=domains.ielts.stores[IELTS_STORE_NAMES.frozenAssessments]||[];
  domains.ielts.stores[IELTS_STORE_NAMES.testBlueprints||'ieltsTestBlueprints']=[];
  domains.ielts.stores[IELTS_STORE_NAMES.testRuns||'ieltsTestRuns']=[];
  domains.ielts.database=IELTS_DB_NAME;domains.ielts.databaseVersion=IELTS_DB_VERSION;
...
function upgradeRegistryV5Envelope(input){
  ...
  const expectedRows=BACKUP_STORE_REGISTRY.filter(row=>!(row.owner==='ielts'&&['ieltsTestBlueprints','ieltsTestRuns'].includes(row.store))).map(row=>row.owner==='ielts'?{...row,databaseVersion:3}:row);
