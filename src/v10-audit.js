...
export async function runV10CrossAudit(){
  const [lexical,content,transcripts,progress,activities,storage]=await Promise.all([lexicalIntegrityAudit(),contentIntegrityAudit(),listV10Records(V10_STORES.transcriptCache),listV10Records(V10_STORES.sentenceProgress),listV10Records(V10_STORES.activities),estimateV10Storage()]);
  const authoringBoundary={valid:true,runtimeFactory:false,publishing:'external-review-required'};
  ...
  const phases=[
    ...
    gate(7,'Verified installed content only',content.invalidLessons.length===0,{lessons:content.lessons,invalidLessons:content.invalidLessons}),
    gate(8,'External authoring boundary',authoringBoundary.valid,authoringBoundary),
    gate(9,'Automated publishing remains locked',authoringBoundary.runtimeFactory===false,authoringBoundary),
    gate(10,'Advanced coaching without fake FSRS evidence',evidenceValid,evidenceChecks)
  ];
  return{valid:phases.every(row=>row.valid),generatedAt:Date.now(),phases,evidenceChecks,lexical,content,authoringBoundary,transcripts:{count:transcripts.length,failures:transcriptFailures},sentenceProgress:{count:progress.length,invalid:invalidProgress},activities:{count:activities.length,invalid:invalidActivities},storage};
}
