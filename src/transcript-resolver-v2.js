...
async function localCacheProvider(context){const row=await getV10Record(V10_STORES.transcriptCache,context.cacheKey);if(!row?.segments?.length)throw new Error('cache-miss');return{...row,provider:'indexeddb',cached:true};}
