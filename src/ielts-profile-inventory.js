...
function validateReadingMatchingPayload(payload,{id,sourceRevisionRef,profiles,questionBinding}){
  const bindingMatching=IELTS_READING_MATCHING_KINDS.includes(questionBinding.kind),payloadMatching=IELTS_READING_MATCHING_KINDS.includes(payload?.kind);if(bindingMatching!==payloadMatching)throw new TypeError('Reading matching payload and binding kinds must match exactly.');if(!bindingMatching)return;
